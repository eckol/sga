<x-app-layout>
    ...
    <style>
        ...
        select[name="tabla-indicadores-faltas_length"] {
            border-radius: 5px !important;
        }
    </style>

    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestión de Indicadores de Faltas</h2>
    </x-slot>
    ...
    <div class="card card-body p-2">
        <div class="d-flex justify-content-between mb-2">
            ...
            <button class="btn btn-primary btn-sm" style="font-size: 0.7rem;" data-bs-toggle="modal"
                data-bs-target="#modalCrear">+ Nueva Falta</button>
        </div>

        <table id="tabla-indicadores-faltas" class="table table-sm table-hover table-bordered table-xs">
                ...
                @foreach($faltas as $falta)
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <form action="{{ route('indicadores_faltas.destroy', $falta->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                                @method('DELETE')
    ...
    <div class="modal fade" id="modalCrear" tabindex="-1">
        <div class="modal-dialog modal-sm">
            <form action="{{ route('indicadores_faltas.store') }}" method="POST" class="modal-content">
                @csrf
    ...
    <div class="modal fade" id="modalEditarFalta" tabindex="-1">
        <div class="modal-dialog">
            <form id="formEditar" method="POST" class="modal-content">
                @csrf @method('PATCH')
                <div class="modal-header p-2 bg-primary text-white">
                    <h6 class="modal-title">Modificar Falta</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-3">
                    <div class="mb-2">
                        <label class="form-label mb-0 fw-bold">Falta</label>
                        <input type="text" name="tipo_falta" id="edit_tipo_falta" class="form-control form-control-sm"
                            required>
                    </div>
                </div>
                <div class="modal-footer p-1">
    ...
    <script>
        ...
        window.onload = function () {
            if (window.jQuery) {
                ...
                var table = $('#tabla-indicadores-faltas').DataTable({
                    "order": [[0, "asc"]],
                    "pageLength": 10,
                    "language": {
                        "search": "Buscar:",
