<x-app-layout>
    ...
    <style>
        ...
        select[name="tabla-estadosciviles_length"] {
            border-radius: 5px !important;
        }
    </style>

    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestión de Estados Civiles</h2>
    </x-slot>
    ...
    <div class="card card-body p-2">
        <div class="d-flex justify-content-between mb-2">
            ...
            <button class="btn btn-primary btn-sm" style="font-size: 0.7rem;" data-bs-toggle="modal"
                data-bs-target="#modalCrear">+ Nuevo Estado Civil</button>
        </div>

        <table id="tabla-estadosciviles" class="table table-sm table-hover table-bordered table-xs">
    ...
    <div class="modal fade" id="modalCrear" tabindex="-1">
        <div class="modal-dialog modal-sm">
            <form action="{{ route('estadosciviles.store') }}" method="POST" class="modal-content">
                @csrf
                <div class="modal-header p-2 bg-primary text-white">
                    <h6 class="modal-title">Nuevo Estado Civil</h6>
                </div>
                <div class="modal-body p-2">
                    <input type="text" name="estado_civil" class="form-control form-control-sm"
                        placeholder="Nombre del Estado Civil" required>
                </div>
    ...
    <div class="modal fade" id="modalEditarEstadoCivil" tabindex="-1">
        <div class="modal-dialog">
            <form id="formEditar" method="POST" class="modal-content">
                @csrf @method('PATCH')
                <div class="modal-header p-2 bg-primary text-white">
                    <h6 class="modal-title">Modificar Estado Civil</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-3">
                    <div class="mb-2">
                        <label class="form-label mb-0 fw-bold">Estado Civil</label>
                        <input type="text" name="estado_civil" id="edit_estado_civil"
    ...
    <script>
        ...
        window.onload = function () {
            if (window.jQuery) {
                ...
                var table = $('#tabla-estadosciviles').DataTable({
                    "order": [[0, "asc"]],
                    "pageLength": 10,
                    "language": {
                        "search": "Buscar:",
                ...
                $(document).on('click', '.btn-editar', function (e) {
                    ...
                    // Llenar campos
                    $('#formEditar').attr('action', '/estadosciviles/' + id);
                    $('#edit_estado_civil').val(estado_civil);

                    // Forzar apertura del modal
                    var myModal = new bootstrap.Modal(document.getElementById('modalEditarEstadoCivil'));
                    myModal.show();
                });
            } else {
