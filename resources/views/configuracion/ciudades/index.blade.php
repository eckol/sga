<x-app-layout>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button type="button" class="btn btn-primary btn-xs py-0 px-1 btn-editar"
                                style="font-size: 0.65rem;" data-id="{{ $ciudad->id }}" data-ciudad="{{ $ciudad->ciudad }}">
                                Editar
                            </button>

                            <form action="{{ route('ciudades.destroy', $ciudad->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
    ...
    <div class="modal fade" id="modalEditarCiudad" tabindex="-1">
        <div class="modal-dialog">
            <form id="formEditar" method="POST" class="modal-content">
                @csrf @method('PATCH')
                <div class="modal-header p-2 bg-primary text-white">
                    <h6 class="modal-title">Modificar Ciudad</h6>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-3">
                    <div class="mb-2">
                        <label class="form-label mb-0 fw-bold">Ciudad</label>
                        <input type="text" name="ciudad" id="edit_ciudad" class="form-control form-control-sm" required>
                    </div>
                </div>
                <div class="modal-footer p-1">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancelar</button>
        ...
        window.onload = function () {
            if (window.jQuery) {
                ...
                $(document).on('click', '.btn-editar', function (e) {
                    ...
                    var myModal = new bootstrap.Modal(document.getElementById('modalEditarCiudad'));
                    myModal.show();
                });
            } else {
