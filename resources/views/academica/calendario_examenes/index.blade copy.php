<x-app-layout>
        ...
        <div class="max-w-full mx-auto px-4">
            ...
            <div class="filter-card p-3 mb-4 shadow-sm">
                <div class="row g-3 align-items-end">
                    ...
                    {{-- Botones de exportación y envío por email --}}
                    <div class="col-md-5">
                        <label class="form-label small fw-bold text-muted mb-1">
                            <i class="fas fa-cogs me-1"></i>Acciones del Calendario:
                        </label>
                        <div class="d-flex gap-2 flex-wrap btn-export-group">
                            ...
                            {{-- NUEVO BOTÓN: Reemplaza el texto informativo anterior --}}
                            <button id="btnEnviarEmailsBloque" class="btn btn-warning btn-sm shadow-sm">
                                <i class="fas fa-paper-plane me-1"></i> Enviar por Mail
                            </button>
                        </div>
                    </div>
                </div>
            ...
            @php
                ...
                foreach ($examenes as $ex) {
                    $pivot[$ex->ciclo][$ex->etapa][$ex->tipo_prueba][$ex->fecha->toDateString()][$ex->grado_curso_id] = $ex;
                }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ─────────────────────────────────────────────────────────────────────
        // ACCIÓN: ENVÍO MANUAL DE EMAILS POR BLOQUE SELECCIONADO
        // ─────────────────────────────────────────────────────────────────────
        document.getElementById('btnEnviarEmailsBloque').addEventListener('click', async function() {
            const ciclo = document.getElementById('filterCiclo').value;
            const etapa = document.getElementById('filterEtapa').value;
            const tipo  = document.getElementById('filterTipo').value;

            // Validación de seguridad para asegurar que hay un bloque definido seleccionado
            if (!ciclo || !etapa || !tipo) {
                toast('⚠ Por favor, selecciona un Ciclo, Etapa y Tipo específicos para realizar el envío.', false);
                return;
            }

            const mensajeConfirmacion = `¿Confirmas el envío por email del calendario de exámenes?\n\n• Ciclo: ${ciclo}\n• Etapa: ${etapa}\n• Tipo: ${tipo}\n\nEsta acción notificará de inmediato a todos los responsables de los alumnos inscriptos en estos cursos.`;
            ...
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Procesando y Encolando...';

            try {
                const response = await fetch("{{ route('academica.calendario-examenes.notificar-bloque') }}", {
                    method: 'POST',
                    headers: {
                ...
                console.error(error);
                toast('✘ Ocurrió un error de comunicación con el servidor al procesar el envío.', false);
            } finally {
                btn.disabled = false;
