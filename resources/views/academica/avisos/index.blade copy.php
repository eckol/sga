    ...
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-12 d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="fas fa-bullhorn me-2"></i>Avisos y Circulares</h5>
                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalNuevoAviso">
        ...
        <div class="card">
            <div class="card-body p-2">
                <table id="tablaAvisos" class="table table-sm table-hover table-bordered mb-0">
                    ...
                    <tbody>
                        @foreach($avisos as $aviso)
                            <tr>
                                ...
                                <td>{{ optional($aviso->colaborador)->nombre_completo ?? '-' }}</td>
                                <td>
                                    @php
                                ...
                                <td class="text-center">
                                    <button class="btn btn-info btn-xs btn-ver-aviso" data-id="{{ $aviso->id }}"
                                        title="Ver detalle">
        ...
        <div class="mt-2">{{ $avisos->links() }}</div>
    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer py-1">
                    ...
                    <button type="button" class="btn btn-primary btn-sm" id="btnEnviarAviso">
        ...
        <script>
            $(function () {
                ...
                $('#btnEnviarAviso').on('click', function () {
                    ...
                    // Limpiar campos deshabilitados según destino
                    const tipo = $('#destinoTipo').val();
                    if (tipo === 'colegio_completo') formData.delete('destino_id');
                    if (tipo === 'ciclo') formData.delete('destino_id'); // se re-agrega del select correcto
                    if (tipo === 'grado_curso') formData.delete('destino_id');

                    // Re-agregar destino_id del selector activo
                    if (tipo === 'ciclo') {
                ...
                $(document).on('click', '.btn-ver-aviso', function () {
                    ...
                    $.get(`{{ url('academica/avisos') }}/${id}`, function (data) {
                        ...
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-9">${data.aviso.fecha}</dd>
                            <dt class="col-sm-3">Destino</dt>
                            <dd class="col-sm-9">${data.destino_label}</dd>
                            <dt class="col-sm-3">Enviado por</dt>
                            <dd class="col-sm-9">${data.colaborador}</dd>
                            <dt class="col-sm-3">Estado</dt>
                            <dd class="col-sm-9">${data.aviso.estado} (${data.aviso.total_enviados} correos)</dd>
                            <dt class="col-sm-3">Mensaje</dt>
                            <dd class="col-sm-9">${data.aviso.mensaje ? data.aviso.mensaje.replace(/\n/g, '<br>') : '<em>Sin mensaje</em>'}</dd>
                            <dt class="col-sm-3">Adjunto</dt>
                            <dd class="col-sm-9">${adjuntoHtml}</dd>
                        </dl>
                    `);
                    });
                });
