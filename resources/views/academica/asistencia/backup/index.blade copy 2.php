    ...
    <script>
        window.onload = function () {
            ...
            $(document).on('click', '.btn-asist:not(.finde):not(.feriado)', function () {
                ...
                // Solo actualizar en memoria y en el DOM.
                // El guardado real ocurre únicamente al presionar "Guardar asistencia".
                ...
                // Marcar la fila como modificada para feedback visual
            ...
            $('#btn-guardar-grilla').on('click', function () {
                ...
                // Recorrer TODOS los botones hábiles (no feriado, no finde).
                // Se envían tanto los que tienen estado como los vacíos (''),
                // para que el backend pueda eliminar los que quedaron en blanco.
                $('.btn-asist:not(.finde):not(.feriado)').each(function () {
                    registros.push({
                        inscripcion_id: $(this).data('inscripcion'),
                ...
                if (registros.length === 0) {
                    Swal.fire('Sin cambios', 'No hay días hábiles en la grilla.', 'info');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        // Quitar el marcado visual de filas modificadas
                        $('tr.table-warning').removeClass('table-warning');
                        Swal.fire('Guardado', `${res.guardados} guardados, ${res.eliminados} eliminados.`, 'success');
                    },
                    error: function (xhr) {
                        const msg = xhr.responseJSON?.message || 'No se pudo guardar la asistencia.';
                        Swal.fire('Error', msg, 'error');
                    },
                    complete: function () {
                        $btn.prop('disabled', false).html('<i class="fas fa-save me-1"></i>Guardar asistencia');
                    }
                });
            });
