<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Registros Anecdóticos</h2>
    </x-slot>
    ...
    <div class="card card-body p-2 shadow-sm mb-3">
        <form method="GET" action="{{ route('academica.registros-anecdoticos.index') }}" id="formFiltro">
            <div class="row align-items-center g-2">
                ...
                <div class="col-md-auto">
                    <a href="{{ route('academica.registros-anecdoticos.index') }}"
    ...
    <div class="card card-body p-2 shadow-sm">
        <table id="tabla-registros" class="table table-sm table-hover table-bordered">
            ...
            <tbody>
                @foreach($registros as $r)
                    <tr>
                        ...
                        <td>{{ $r->gradoCurso->gradocurso ?? '—' }}</td>
                        <td>{{ $r->alumno ? $r->alumno->apellidos . ', ' . $r->alumno->nombres : '—' }}</td>
                        <td>{{ $r->asignatura->asignatura ?? '—' }}</td>
                        <td>{{ \Str::limit($r->detalle, 80) }}</td>
                        <td class="text-center">
                            <button type="button" class="btn btn-warning btn-xs py-0 px-1 btn-editar"
                                style="font-size:0.65rem" data-id="{{ $r->id }}"
                                data-fecha="{{ \Carbon\Carbon::parse($r->fecha)->format('Y-m-d') }}"
                                data-grado="{{ $r->grado_curso_id }}" data-alumno="{{ $r->alumno_id }}"
                                data-asignatura="{{ $r->asignatura_id }}" data-detalle="{{ e($r->detalle) }}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button type="button" class="btn btn-danger btn-xs py-0 px-1 btn-eliminar"
                                style="font-size:0.65rem" data-id="{{ $r->id }}"
                                data-desc="{{ $r->alumno ? $r->alumno->apellidos . ' ' . $r->alumno->nombres : '' }} — {{ \Str::limit($r->detalle, 40) }}">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header modal-header-cst">
                    <h6 class="modal-title"><i class="fas fa-plus-circle me-1"></i> Nuevo Registro Anecdótico</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form method="POST" action="{{ route('academica.registros-anecdoticos.store') }}">
                    @csrf
                        ...
                        <div class="row g-2">
                            ...
                            <div class="col-md-12">
                                <label class="form-label form-label-sm fw-bold">Detalle / Observación <span
                                        class="text-danger">*</span></label>
                                <textarea name="detalle" id="crear_detalle" class="form-control form-control-sm"
                                    rows="3" required
                                    placeholder="Ej: No presenta tarea, Falta el respeto a la profesora..."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-body">
                        <div class="row g-2">
                            ...
                            <div class="col-md-12">
                                <label class="form-label form-label-sm fw-bold">Detalle / Observación <span
                                        class="text-danger">*</span></label>
                                <textarea name="detalle" id="editar_detalle" class="form-control form-control-sm"
    ...
    <div class="modal fade" id="modalEliminar" tabindex="-1">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header modal-header-cst">
                    <h6 class="modal-title"><i class="fas fa-trash me-1"></i> Eliminar Registro</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body" style="font-size:0.85rem">
                    ¿Eliminar el registro de <strong id="eliminar_desc"></strong>?
                </div>
                <div class="modal-footer py-1">
    ...
    @push('scripts')
        ...
        <script>
            window.onload = function () {
                ...
                // ── DataTable ──
                var t = $('#tabla-registros').DataTable({
                    paging: true,
                    pageLength: 25,
                    order: [[2, 'desc']],
                    ...
                    language: {
                        ...
                        emptyTable: 'No hay registros anecdóticos cargados',
                    ...
                    buttons: [
                        {
                            ...
                            messageTop: 'Registros Anecdóticos',
                ...
                $(document).on('click', '.btn-editar', function () {
                    ...
                    $('#formEditar').attr('action', '{{ url("academica/registros-anecdoticos") }}/' + id);
                    $('#editar_fecha').val(fecha);
                    $('#editar_detalle').val(detalle);
                    $('#editar_grado').val(gradoId);

                    // Cargar alumnos y asignaturas con valores preseleccionados
                ...
                $(document).on('click', '.btn-eliminar', function () {
                    ...
                    $('#formEliminar').attr('action', '{{ url("academica/registros-anecdoticos") }}/' + id);
