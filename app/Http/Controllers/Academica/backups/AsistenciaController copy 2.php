...
class AsistenciaController extends Controller
{
     ...
     * Guarda el mes completo de un grado en un solo request.
     * Registros con estado vacío ('') se eliminan de la BD si existían.
    ...
    public function guardarGrilla(Request $request): JsonResponse
    {
        ...
        $todos      = collect($request->registros);
        $conEstado  = $todos->filter(fn($r) => !empty($r['estado']) && in_array($r['estado'], ['Presente','Ausente','Justificado','Tardanza']));
        $sinEstado  = $todos->filter(fn($r) => empty($r['estado']));

        // ── Upsert de los registros con estado ───────────────────────
        $guardados = 0;
        if ($conEstado->isNotEmpty()) {
            $rows = $conEstado->map(fn($r) => [
            ...
            Asistencia::upsert(
                ...
                ['estado', 'observacion', 'registrado_por', 'updated_at']
            );
            $guardados = count($rows);
        }

        // ── Eliminar los registros sin estado (vaciados por el usuario) ──
        $eliminados = 0;
        if ($sinEstado->isNotEmpty()) {
            foreach ($sinEstado as $r) {
                $eliminados += Asistencia::where('inscripcion_id', $r['inscripcion_id'])
                    ->where('fecha', $r['fecha'])
                    ->delete();
            }
        }
    ...
    public function porAlumno(int $alumnoId, Request $request): JsonResponse
    {
        ...
        $inscripcion = Inscripcion::where('alumno_cid',
